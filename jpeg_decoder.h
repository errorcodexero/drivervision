...
namespace Jpeg
{
    ...
    public:
        ...
        // if IsColor() then 32 bit as R,G,B,alpha bytes
    ...
    private:
        ...
        inline void _DecodeSOF(void) {
            ...
            if (ctx.ncomp == 3) {
		// 4 components, not just 3, because we need an (empty) alpha channel for IMAQ RGB
                ctx.rgb = (unsigned char*)AllocMem(ctx.width * ctx.height * 4);
        ...
        inline void _Convert() {
			...
			// IMAQ image wants these values in a strange order
                        *prgb++ = _Clip((y + 454 * cb            + 128) >> 8);	// blue
                        *prgb++ = _Clip((y -  88 * cb - 183 * cr + 128) >> 8);	// green
                        *prgb++ = _Clip((y            + 359 * cr + 128) >> 8);	// red
			*prgb++ = 0;						// alpha
                    }
...
inline size_t Decoder::GetImageSize(void) const { return ctx.width * ctx.height * (ctx.ncomp == 1 ? 1 : 4); }
